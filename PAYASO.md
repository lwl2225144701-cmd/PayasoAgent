...
| 全量确定性套件（无 LLM，82 个） | `npm run test:all` | ~17s（有界并发） |
...
* 需要 ≥3 步或多次工具调用的任务，**先用 `updatePlan` 列出计划再动手**（单步任务不要建：多一次调用是纯开销）。
* 开始一项标 `in_progress`，完成立刻标 `completed` 并重发**完整清单**；同时最多一项 `in_progress`。
* 标题写给用户看（「跑通 runtime-boundary 套件」），不要写工具参数细节 —— 前端会把它渲染成进度清单。
* 计划是承诺而非硬约束：任务目标变了就改清单，别为了"清空待办"做无意义的收尾。
