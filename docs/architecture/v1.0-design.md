...
> **⚠️ 本文档已过时（Superseded）。** v1.0 设计稿，目录结构与模块清单均为旧版。**当前架构以 [architecture-current.md](./architecture-current.md) 为准**。本文件仅保留作为设计历史记录。
...
| | `src/tools.ts` | 92 | Tool 注册与执行：`register`/`execute`/`getSchemas` + calculator（失败抛异常）+ getWeather（mock） |
| **Evaluation** | `src/test.ts` | 220 | 端到端能力验证：子进程跑 14 个真实任务，断言输出与工具顺序/禁调，汇总 PASS/FAIL |
...
14 个真实端到端任务，子进程隔离运行（避免状态互相干扰），断言输出关键词 + 工具调用顺序 + 禁用工具：
...
| 11 | 多工具选择 | 天气任务只走 getWeather | `深圳天气` → 28°C ✅ |
| 12 | 工具路由 | 计算任务只走 calculator | `15*37` → 555 ✅ |
| 13 | 工具串联 | 先 getWeather 再 calculator | 28°C+10 → 38 ✅ |
| 14 | 跨工具依赖失败 | 上游失败 → 下游中止，calculator 0 次调用 | 不存在城市 → 说明无法计算 ✅ |
| 15 | 工具成功但结果无效 | 识别 NaN/未定义，不编造数值 | `0/0`+10 → 说明未定义 ✅ |

**实测：15/15 PASS（~1m45s）**。注：任务 9 依赖 `llm.ts` 中的模拟中断开关（当前已注释，取消注释才能跑全量；注释态下偶发不稳定）；偶发 LLM API 慢会导致个别任务超时，重跑即可。
