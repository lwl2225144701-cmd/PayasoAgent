...
### 8.4 真实浏览器验收结果（用户实测，2026-09-11）

由用户在运行中的应用上手工验收并回传。**这是本节唯一的真实浏览器证据**，
其余条目均为代码/单测层面。

| 项 | 结果 | 证据 |
|---|---|---|
| **B1 终态长会话 SSE = 0** | ✅ **通过** | 加载含 5 个终态 Run 的会话，Network 中 Run 事件**全部**是 `/events/snapshot`，`event-stream` 数量为 **0** |
| B5 fence 不泄漏 | ✅ 通过 | Running 状态下代码已是 `<code>`，可见文本无三反引号 |

**B1 的意义**：这是 §3.1（P0-1）唯一直接的验收指标 —— 改造前该数字等于会话的回合数
（实测样本是 25），现在为 0。P0-1 至此可以认为**已被真实环境证实**，而非仅代码推断。

**仍未取到的项**：

- ❌ **B3（>50ms 长任务数）**：完整录制了 22 秒真实流式，但关闭 Screenshots 后停录仍卡在
  DevTools 的 trace processing，无法可信读取。**必须换一种取数方式**（在页面内用
  `PerformanceObserver` 直接统计 `longtask` / `long-animation-frame` 的 `blockingDuration`，
  绕开 trace 解析）。
- ⚠️ **A2/A4 只有观感结论**：「运行态与终态已完成部分的语义节点一致、未发现整段消失重建」
  「流式与终态均保持在末尾、无明显落后一帧」，但**没有量化**（未捕获帧级位移、未记录滚动位置）。
  若要宣称「零跳动」，需录屏或提取 `layout-shift` / 逐帧采样距底部距离。
