...
| Windows ACL 执行器 | ✅ 代码就绪 / ⏸ 待真机验证 | `windows-acl-sandbox.ts`（argv 构造、exit 127+签名双条件失败识别、taskkill 整树终止） |
...
| 测试 | ✅ 90 套件全绿 | `shell-executor`（平台矩阵 + fail-closed E2E：macOS 上真跑 runner → kernel32 加载失败 → 127+签名 → 结构化报错）、`windows-acl-sandbox`（argv/失败契约/假 spawn） |
...
- **read-only** → `AclSandbox{ mode:'workspace-write', writableDirs:[scratch], writeSid:tempWriteSid(scratch), tempDir:null }`：workspace 不授权 → 只读；历史 workspace-write 留下的 standing workspace ACE 因 `workspaceWriteSid` 不在 restricting 列表而惰性
- **workspace-write / full-access** → 库原生形状：`writableDirs:[workspace] + workspaceWriteSid(workspace)`（standing ACE）+ `tempDir:scratch + tempWriteSid(scratch)`（可撤销 ACE）

runner 以子进程方式运行（Payaso 主进程不加载任何 Win32 FFI）；koffi 自绑 `SetEnvironmentVariableW` 设 `HOME/TMP/TEMP/TMPDIR=scratch`（显式 env block 过 `CreateProcessAsUserW` 会 `ERROR_INVALID_PARAMETER`）。失败契约：`payaso-win-acl: <detail>` + exit 127 → 命令未执行，fail-closed 不回退。

**已知偏差（相对原方案）**：`revokeWorkspaceStandingAce` 清理工具未实现——库不导出 `revokeWrite`，用公共 API 无法撤销 standing ACE；本期以文档披露 + `icacls` 手动清理代替（见 §5 第 8 条），不为此重写 Win32 DACL 手术代码。
...
- Windows ACL 初始化或 runner 启动失败，不回退裸跑（exit 127 + `payaso-win-acl: ` 签名 → 结构化错误）。既有 `PAYASO_SHELL_UNSANDBOXED` 开关仍是独立显式选择。
- 工作区 ACE 持续保留（合成 SID，无账户映射，跨会话复用）；临时 ACE 可撤销（dispose）。scratch 删除在 runner 退出（子进程已结束）之后由 runtime-tools 执行。standing ACE 清理：`icacls <workspace> /remove <SID>`（SID 由 `workspaceWriteSid` 派生规则推导；真机验证清单第 8 条）。
...
7. **runner 失败路径**：坏 argv / 删除 runner / 指向不存在目录 → exit 127 + `payaso-win-acl: ` 前缀、命令未执行、Payaso 侧结构化错误（不回退无沙箱）。
8. **standing ACE 清理**：`icacls <workspace> /remove <workspaceWriteSid 推导的 SID>` 验证 DACL 恢复；清理后再次 workspace-write 仍能重新授权。
