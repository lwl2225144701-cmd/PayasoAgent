...
| **Evaluation** | `src/test.ts` | 201 | 端到端能力验证：子进程跑 13 个真实任务，断言输出与工具顺序，汇总 PASS/FAIL |
...
13 个真实端到端任务，子进程隔离运行（避免状态互相干扰），断言输出关键词 + 工具调用顺序：
...
**实测：13/13 PASS（~1m40s）**。注：任务 9 依赖 `llm.ts` 中的模拟中断开关（当前已注释，取消注释才能跑全量；注释态下偶发不稳定）。
