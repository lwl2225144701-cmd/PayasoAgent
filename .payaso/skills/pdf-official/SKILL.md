---
name: pdf-official
description: PDF 处理工具集（Node 生态：pdfjs-dist 提取 + pdf-lib 变换/表单）。分诊/提取/合并/拆分/旋转/表单探测/表单填充/覆盖文本/校验已可用；OCR、页渲染待迁移。附件 PDF 提取 partial/failed、或需处理原件时使用。
version: 2.1.0
---
...
| 完整性校验 | `sanity_check.mjs` | ✅ Node（pdf-lib load/save 往返） |
...
| 表单探测 | `node scripts/probe_fields.mjs 表单.pdf` |
| 表单填充 | `node scripts/apply_values.mjs 表单.pdf values.json --out out.pdf [--flatten]` |
| 覆盖文本 | `node scripts/overlay_text.mjs 文件.pdf entries.json --out out.pdf [--font CJK.ttf]` |
| 完整性校验 | `node scripts/sanity_check.mjs 文件.pdf` |
...
- `form_field_count > 0` → 有表单字段：`probe_fields.mjs` 探测 → `apply_values.mjs` 填充
  （非法值会拒绝，exit 3）→ 需要固定值时加 `--flatten`。
