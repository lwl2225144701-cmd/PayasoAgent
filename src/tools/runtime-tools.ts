...
register({
  ...
  execute: async (args, context) => {
    if (context.writeScope !== undefined) throw new Error('受限任务不能创建目录');
    const rel = String(args.path ?? '').trim();
...
async function executeContainedShell(
  ...
  try {
    const result = await executeShellCommand({
      ...
      permissionMode: context.writeScope !== undefined ? 'read-only' : context.permissionMode,
...
register({
  ...
  description:
    ...
    `限定修改文件时，检查用 node -e 等内联命令，不新建脚本或日志。允许新增文件时，长命令输出重定向后用 read/grep 复查，勿为换切片重复执行。` +
