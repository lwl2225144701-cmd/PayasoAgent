...
export function protectRuntimeObserver(observer: RuntimeObserver): RuntimeObserver {
  return {
    log: (message) => { try { observer.log(message); } catch {} },
    state: (state, detail) => { try { observer.state(state, detail); } catch {} },
    scratchpad: (scratchpad) => { try { observer.scratchpad(scratchpad); } catch {} },
    traceEvent: (event) => { try { observer.traceEvent(event); } catch {} },
    trace: (trace) => { try { observer.trace(trace); } catch {} },
  };
}
