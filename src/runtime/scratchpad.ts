...
export function setNextStep(pad: Scratchpad, next: { tool: string; input: string }): void {
...
export function recordFailure(
  ...
  const existing = pad.failedSteps.find((f) => f.tool === fail.tool && f.input === fail.input);
  if (existing) {
    existing.retries += 1;
...
export function clearFailure(pad: Scratchpad, tool: string, input: string): void {
  pad.failedSteps = pad.failedSteps.filter((f) => !(f.tool === tool && f.input === input));
}
...
export function recordInvalid(
  ...
  pad.lastResult =
    typeof invalid.result === 'string' ? invalid.result : JSON.stringify(invalid.result);
}
