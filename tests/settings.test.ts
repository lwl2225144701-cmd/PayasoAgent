...
async function getRunStatus(runId: string): Promise<{ status: string }> {
  const body = await (
    await fetch(`${base}/runs/${runId}`, { headers: await authHeaders() })
  ).json();
...
async function postJSONWithOrigin(
  url: string,
  body: any,
  origin: string,
...
(async () => {
  // 1. 默认列表：新库无内置模板，列表为空
  const list = await getJSON(`${base}/settings/models`);
  ...
  check(
    'template add has key',
    tplAdded.hasApiKey === true && tplAdded.apiKeyMasked === '********',
  );
  const afterTpl = await getJSON(`${base}/settings/models`);
  check(
    'template add visible in list',
    afterTpl.models.some((m: any) => m.id === tplAdded.id && m.hasApiKey === true),
  );
  ...
  check(
    'POST dup name 400',
    dupName.message ===
      'Provider with same name or baseUrl already exists: TestProvider (https://api.test.com)',
  );

  // 4. 重复 baseUrl 拒绝
  const dupUrl = await postJSON(
    `${base}/settings/models`,
    {
      name: 'Other',
      baseUrl: 'https://api.test.com',
  ...
  const updated = await patchJSON(`${base}/settings/models/${created.id}`, {
    name: 'TestProvider Updated',
  });
  check('PATCH name updated', updated.name === 'TestProvider Updated');
  check(
    'PATCH apiKey unchanged',
    updated.apiKeyMasked === '********' && updated.hasApiKey === true,
  );

  // 9. 更新清除 apiKey（传 null）
  const cleared = await patchJSON(`${base}/settings/models/${created.id}`, {
    apiKey: null,
  });
  check('PATCH apiKey cleared', cleared.apiKeyMasked === '****' && cleared.hasApiKey === false);

  // 10. 更新替换 apiKey
  const replaced = await patchJSON(`${base}/settings/models/${created.id}`, {
    apiKey: 'new-key-9999',
  });
  check(
    'PATCH apiKey replaced',
    replaced.apiKeyMasked === '********' && replaced.hasApiKey === true,
  );

  // 11. 空字符串 apiKey 拒绝
  const emptyKey = await patchJSON(
  ...
  // 12. 非法 ID 拒绝
  const badId = await getJSON(`${base}/settings/models/not-a-uuid`, 400);
  check('GET bad id 400', badId.message === 'invalid_model_id');

  const badIdPatch = await patchJSON(`${base}/settings/models/not-a-uuid`, { name: 'x' }, 400);
  check('PATCH bad id 400', badIdPatch.message === 'invalid_model_id');

  const badIdDel = await delJSON(`${base}/settings/models/not-a-uuid`, 400);
  check('DELETE bad id 400', badIdDel.message === 'invalid_model_id');

  // 13. 删除不存在的 provider 返回 404（内置模板已移除，无"不可删内置"语义）
  const delMissingStatus = await (
    await fetch(`${base}/settings/models/${crypto.randomUUID()}`, {
      method: 'DELETE',
  ...
  const deleted = await delJSON(`${base}/settings/models/${created.id}`);
  check('DELETE returns deleted:true', deleted.deleted === true);

  // 15. 删除后不存在
  const afterDel = await getJSON(`${base}/settings/models/${created.id}`, 404);
  check('GET after delete 404', afterDel.error === 'not_found');

  // 16. 列表最终：DeepSeek + Dedup 两个自定义（无内置模板）
  const finalList = await getJSON(`${base}/settings/models`);
  check('final list count', finalList.models.length === 2);
  check(
    'final list builtin count',
    finalList.models.filter((m: any) => m.kind === 'builtin').length === 0,
  );

  // 17. 设置默认模型：校验矩阵（未知 404 / 空 400 / 未配置密钥 400）
  const unknownDefault = await postJSON(
    `${base}/settings/default`,
    { providerId: 'not-exist', model: 'm' },
    404,
  );
  check('default unknown provider 404', unknownDefault.error === 'not_found');

  const emptyDefault = await postJSON(`${base}/settings/default`, { providerId: '   ' }, 400);
  ...
  const unconfiguredDefault = await postJSON(
    `${base}/settings/default`,
    { providerId: noKeyProv.id, model: 'nk-chat' },
    400,
  );
  check(
    'default unconfigured provider 400',
    unconfiguredDefault.message === 'Provider has no API key configured',
  );
  ...
  const badModelDefault = await postJSON(
    `${base}/settings/default`,
    { providerId: defaultProvider.id, model: 'not-in-catalog' },
    400,
  );
  check(
    'default unknown model 400',
    badModelDefault.message === 'Model "not-in-catalog" is not in provider catalog',
  );

  const okDefault = await postJSON(
    `${base}/settings/default`,
    { providerId: defaultProvider.id, model: 'dm-reasoner' },
    200,
  );
  check(
    'default set returns pair',
    okDefault.defaultProviderId === defaultProvider.id &&
      okDefault.defaultModelId === 'dm-reasoner',
  );

  const currentDefault = await getJSON(`${base}/settings`);
  check(
    'GET settings returns pair',
    currentDefault.defaultProviderId === defaultProvider.id &&
      currentDefault.defaultModelId === 'dm-reasoner',
  );
  ...
  const afterAdd = await getJSON(`${base}/settings`);
  check(
    'default survives addModel',
    afterAdd.defaultProviderId === defaultProvider.id && afterAdd.defaultModelId === 'dm-reasoner',
  );

  await patchJSON(`${base}/settings/models/${defaultProvider.id}`, {
    name: 'DefaultProvider Renamed',
  });
  const afterUpdate = await getJSON(`${base}/settings`);
  check(
    'default survives updateModel',
    afterUpdate.defaultProviderId === defaultProvider.id &&
      afterUpdate.defaultModelId === 'dm-reasoner',
  );

  // 20. 删除默认 provider → 默认对清空（解析时回退第一个可用 provider）
  await delJSON(`${base}/settings/models/${defaultProvider.id}`);
  const afterDelete = await getJSON(`${base}/settings`);
  check(
    'default cleared after deleting default provider',
    afterDelete.defaultProviderId === '' && afterDelete.defaultModelId === '',
  );
  ...
  // 21. 环境配置一次性导入（.env → 设置）：新建自定义 Provider 并设为默认
  {
    ...
    check(
      'env import created configured provider',
      !!view && view.hasApiKey && view.models.includes('MiniMax-M3'),
    );
    check(
      'env import set default pair',
      store.getDefaultProviderId() === imported?.providerId &&
        store.getDefaultModelId() === 'MiniMax-M3',
    );

    // 幂等：导入过一次后不再重复（即使传入不同配置）
    const again = store.importEnvFallback({
  ...
  // 22. 环境配置导入：无内置模板，匹配不到时新建自定义 provider（不新建则填已有）
  {
    ...
    const openai = imported
      ? store.listViews().find((p) => p.id === imported.providerId)
      : undefined;
    check(
      'env import filled custom key',
      !!openai && openai.hasApiKey && openai.apiKeyMasked === '********',
    );
    check(
      'env import kept catalog',
      !!openai && openai.models.includes('gpt-4o') && openai.models.length >= 1,
    );
  }
  ...
  // 23. 获取可用模型：OpenAI 兼容 GET /models 的解析（去重、trim、排序、错误分支）
  {
    ...
    try {
      globalThis.fetch = (async () =>
        new Response(
          JSON.stringify({
            data: [{ id: 'm2' }, { id: 'm1' }, { id: ' m1 ' }, { object: 'model' }],
          }),
          { status: 200, headers: { 'Content-Type': 'application/json' } },
        )) as typeof fetch;
      const models = await fetchAvailableModels('https://api.test.com/v1/', 'sk-test');
      check(
        'available models deduped and sorted',
        JSON.stringify(models) === JSON.stringify(['m1', 'm2']),
      );

      globalThis.fetch = (async () =>
        new Response('unauthorized', { status: 401 })) as typeof fetch;
      await assert.rejects(
        () => fetchAvailableModels('https://api.test.com/v1', 'bad'),
        /returned 401/,
      );

      globalThis.fetch = (async () =>
        new Response(JSON.stringify({ data: 'nope' }), { status: 200 })) as typeof fetch;
      await assert.rejects(
        () => fetchAvailableModels('https://api.test.com/v1', 'k'),
        /missing data array/,
      );
    } finally {
    ...
    try {
      ...
      globalThis.fetch = (async (input, init) => {
        if (String(input).includes('api.touch.com')) {
          ...
          return new Response(
            JSON.stringify({ data: [{ id: 'touch-pro' }, { id: 'touch-max' }] }),
            { status: 200 },
          );
        }
      ...
      const viaProvider = await postJSON(
      ...
      const noKey = await postJSON(
        `${base}/settings/available-models`,
        { providerId: noKeyProv.id },
        400,
      );
      check(
        'available-models without key 400',
        noKey.message === 'provider has no API key configured',
      );
    } finally {
      globalThis.fetch = originalFetch;
    }
  ...
  // 25. Secret 隔离：metadata 与凭证分离（Case 1/2/3/4/5/12）
  {
    ...
    const clearedView = (await getJSON(`${base}/settings/models`)).models.find(
      (m: any) => m.id === leak.id,
    );
    ...
    try {
      globalThis.fetch = (async (input, init) => {
        if (String(input).includes('api.sweep-provider.com')) {
          ...
          return new Response(
            JSON.stringify({ choices: [{ message: { role: 'assistant', content: 'swept' } }] }),
            { status: 200 },
          );
        }
        return originalFetch(input, init);
      ...
      check(
        'Case12: run completed through SecretStore credential',
        status12 === 'completed',
        `status=${status12}`,
      );
    } finally {
    ...
    const runJson = JSON.stringify(
      await (await fetch(`${base}/runs/${runId12}`, { headers: await authHeaders() })).json(),
    );
    const eventsText = await (
      await fetch(`${base}/runs/${runId12}/events?live=0`, { headers: await authHeaders() })
    ).text();
    const settingsAll = JSON.stringify(
      await (await fetch(`${base}/settings/models`, { headers: await authHeaders() })).json(),
    );
    const dbAfter = fs.readFileSync(dbPath);
    check(
      'Case12: no secret in run JSON / SSE events / settings / SQLite',
      !runJson.includes(SECRET) &&
        !eventsText.includes(SECRET) &&
        !settingsAll.includes(SECRET) &&
        !dbAfter.includes(SECRET),
    );
  ...
  // 26. Case 11: 删除 Provider → SecretStore 同步清理
  {
    ...
    check(
      'Case11 pre: secret exists',
      secretStore.get(providerSecretKey(del.id)) === 'sk-del-probe',
    );
    await delJSON(`${base}/settings/models/${del.id}`);
    check(
      'Case11: provider deletion cleans secret',
      secretStore.get(providerSecretKey(del.id)) === null,
    );
  }
  ...
  // 27. Case 6/7/8: legacy 明文 apiKey 迁移（先写 SecretStore 再剥 SQLite；幂等；失败不丢 key）
  {
    ...
    legacy.prepare("INSERT INTO settings VALUES ('app', ?)").run(
      JSON.stringify({
        models: [
          {
            id: 'legacy-p1',
          ...
          },
          {
            ...
            kind: 'custom',
            name: 'LegacyTwo',
            baseUrl: 'https://legacy2.example.com',
            apiKey: '',
            hasApiKey: false,
            models: ['l2'],
          },
    ...
    check(
      ...
      mem.get(providerSecretKey('legacy-p2')) === null,
    );
    check(
      'Case6: migrated provider usable',
      store.listModelProviders().some((p: any) => p.id === 'legacy-p1' && p.hasApiKey === true),
    );
    store.close();
    ...
    store2.close();
    check(
      'Case8: second open is a no-op (0 secret writes)',
      secondSetCalls === 0,
      `setCalls=${secondSetCalls}`,
    );
    ...
    flegacy.prepare("INSERT INTO settings VALUES ('app', ?)").run(
      JSON.stringify({
        models: [
          {
            id: 'f-p',
            ...
            apiKey: 'legacy-secret-must-survive',
            hasApiKey: true,
            models: ['f1'],
          },
        ],
    ...
    assert.throws(
      () =>
        new SettingsStore(new DatabaseSync(failingDb), {
          get: () => null,
          set: () => {
    ...
    try {
      globalThis.fetch = (async (input, init) => {
        if (String(input).includes('api.resolution-provider.com')) {
          ...
          return new Response(
            JSON.stringify({ choices: [{ message: { role: 'assistant', content: 'resolved' } }] }),
            { status: 200 },
          );
        }
        return originalFetch(input, init);
      ...
      check(
        'Case9: run completed with merged ModelConfig',
        status9 === 'completed',
        `status=${status9}`,
      );
      check(
        'Case9: Authorization used SecretStore value',
        authHeader === 'Bearer sk-resolution-42',
        `got ${authHeader}`,
      );
  ...
  // 29. SSRF/Origin 防线：available-models 拒绝客户端 baseUrl、要求 providerId、Origin 校验
  {
    ...
    try {
      ...
      // 无 providerId → 400
      const noProvider = await postJSON(
        `${base}/settings/available-models`,
        { baseUrl: 'https://api.openai.com/v1' },
        400,
      );
      ...
      const withBaseUrl = await postJSON(
        `${base}/settings/available-models`,
        { baseUrl: 'https://api.evil.com/v1', apiKey: 'sk' },
        400,
      );
      check(
        'SSRF: client-supplied baseUrl rejected',
        withBaseUrl.message === 'providerId is required',
      );

      // 未知 providerId → 400
      const unknownProv = await postJSON(
        `${base}/settings/available-models`,
        { providerId: 'unknown-id' },
        400,
      );
      check(
        'SSRF: unknown providerId 400',
        unknownProv.message === 'provider not found or not configured',
      );

      // 有 providerId 但无 apiKey → 400（使用未配置密钥的 provider）
      const noKeyProv2 = await postJSON(
        `${base}/settings/models`,
        {
          name: 'NoKeyProv2',
      ...
      const noKey = await postJSON(
        `${base}/settings/available-models`,
        { providerId: noKeyProv2.id },
        400,
      );
      check(
        'SSRF: provider without key 400',
        noKey.message === 'provider has no API key configured',
      );
      ...
      const badOriginRes = await postJSONWithOrigin(
        `${base}/settings/available-models`,
        { providerId: testProv.id },
        'https://evil.com',
      );
    ...
    try {
      globalThis.fetch = (async (input, init) => {
        ...
        if (url.includes('preview-target.example.com')) {
          ...
          return new Response(
            JSON.stringify({ data: [{ id: 'preview-chat' }, { id: 'preview-flash' }] }),
            { status: 200 },
          );
        }
      ...
      const noBaseUrl = await postJSON(
        `${base}/settings/available-models/preview`,
        { apiKey: 'sk-x' },
        400,
      );
      check(
        'preview: missing baseUrl 400',
        noBaseUrl.message === 'baseUrl and apiKey are required',
      );
      const noKey = await postJSON(
        `${base}/settings/available-models/preview`,
      ...
      const ftpUrl = await postJSON(
        `${base}/settings/available-models/preview`,
        { baseUrl: 'ftp://a.com/v1', apiKey: 'sk-x' },
        400,
      );
      check('preview: ftp rejected 400', ftpUrl.message === 'baseUrl protocol not allowed');
      const evilHttp = await postJSON(
        `${base}/settings/available-models/preview`,
        { baseUrl: 'http://api.evil.com/v1', apiKey: 'sk-x' },
        400,
      );
      check(
        'preview: non-loopback http rejected 400',
        evilHttp.message === 'baseUrl protocol not allowed',
      );

      // 正确 https → 200，mock 校验 Authorization 使用表单 apiKey（不落盘、不回显）
      const okHttps = await postJSON(
        `${base}/settings/available-models/preview`,
        {
      ...
      check(
        'preview: https ok 200',
        Array.isArray(okHttps.models) && okHttps.models.includes('preview-chat'),
      );

      // loopback http（开发模式）→ 200
      const okLoopback = await postJSON(
        `${base}/settings/available-models/preview`,
        {
      ...
      check(
        'preview: loopback http ok 200',
        Array.isArray(okLoopback.models) && okLoopback.models.includes('loopback-chat'),
      );

      // Origin 校验同样生效
      const badOrigin = await postJSONWithOrigin(
        `${base}/settings/available-models/preview`,
  ...
  //   (b) metadata 写入失败（SQLite BEFORE UPDATE 触发器 RAISE(ABORT)）—— Secret 已成功写入后的补偿回滚
  {
    ...
    // Case A1: SecretStore.set 失败 → addModel 抛错，无孤儿 secret
    {
      ...
      db.exec(
        'INSERT INTO settings (key, value) VALUES (\'app\', \'{"models":[],"defaultProviderId":"","defaultModelId":""}\')',
      );
      ...
      check(
        'Compensation A1: secret set failure leaves no orphan secret',
        !failingSecretStore.get('model-provider:fail-add-secret:api-key') &&
          failingSecretStore.get('model-provider:fail-add-secret:api-key') === null,
      );
    }
    ...
    // Case A2: SecretStore.set 成功 + metadata 写失败（触发器）→ 补偿删除 Secret
    {
      ...
      check(
        'Compensation A2: orphan secret compensated',
        failingSecretStore.get('model-provider:fail-add-meta:api-key') === null,
        `got ${JSON.stringify(failingSecretStore.get('model-provider:fail-add-meta:api-key'))}`,
      );
      const row = db.prepare("SELECT value FROM settings WHERE key='app'").get() as
        | { value: string }
        | undefined;
      check(
        'Compensation A2: metadata unchanged',
        row ? JSON.parse(row.value).models.length === idBefore : false,
      );
    }
    ...
    // Case B1: SecretStore.set 失败 → updateModel 抛错，secret 保持旧值
    {
      ...
      db2.exec(
        'INSERT INTO settings (key, value) VALUES (\'app\', \'{"models":[{"id":"up1","name":"Up1","baseUrl":"https://up1","hasApiKey":true,"models":["m"],"kind":"custom"}],"defaultProviderId":"","defaultModelId":""}\')',
      );
      ...
      check(
        'Compensation B1: secret write failure keeps old secret',
        failingSecretStore.get('model-provider:up1:api-key') === 'old-secret',
      );
    }
    ...
    // Case B2: 新 Secret 写入成功 + metadata 写失败（触发器）→ 恢复旧 Secret
    {
      ...
      db2.exec('CREATE TABLE settings (key TEXT PRIMARY KEY, value TEXT)');
      db2.exec(
        'INSERT INTO settings (key, value) VALUES (\'app\', \'{"models":[{"id":"up2","name":"Up2","baseUrl":"https://up2","hasApiKey":true,"models":["m"],"kind":"custom"}],"defaultProviderId":"","defaultModelId":""}\')',
      );
      ...
      check(
        'Compensation B2: secret rolled back to old',
        failingSecretStore.get('model-provider:up2:api-key') === 'old-secret',
        `got ${JSON.stringify(failingSecretStore.get('model-provider:up2:api-key'))}`,
      );
      // 默认引用检查在 hasApiKey 更新后执行：清空默认 provider 凭证必须立即清空默认引用
      check(
        'Compensation B2: default reference not dangling',
        JSON.parse(
    ...
    // Case B3: 默认 provider 被清空凭证（apiKey: null）→ 默认引用必须立即清空（顺序修复回归）
    {
      ...
      db2.exec('CREATE TABLE settings (key TEXT PRIMARY KEY, value TEXT)');
      db2.exec(
        'INSERT INTO settings (key, value) VALUES (\'app\', \'{"models":[{"id":"dp1","name":"DP1","baseUrl":"https://dp1","hasApiKey":true,"models":["m1"],"kind":"custom"}],"defaultProviderId":"dp1","defaultModelId":"m1"}\')',
      );
      const store2 = new SettingsStore(db2, failingSecretStore);
      ...
      const row = db2.prepare("SELECT value FROM settings WHERE key='app'").get() as
        | { value: string }
        | undefined;
      const parsed = row
        ? JSON.parse(row.value)
        : { defaultProviderId: 'dp1', defaultModelId: 'm1' };
      check(
        'Default-clear: defaultProviderId cleared',
        parsed.defaultProviderId === '',
        `got ${JSON.stringify(parsed.defaultProviderId)}`,
      );
      check(
        'Default-clear: defaultModelId cleared',
        parsed.defaultModelId === '',
        `got ${JSON.stringify(parsed.defaultModelId)}`,
      );
      check(
    ...
    // Case C1: SecretStore.delete 失败 → metadata 不被删除（防止 orphan secret）
    {
      ...
      const afterRow = db3.prepare("SELECT value FROM settings WHERE key='app'").get() as
        | { value: string }
        | undefined;
    ...
    // Case C2: Secret 删除成功 + metadata 写失败（触发器）→ 恢复 Secret
    {
      ...
      check(
        'Compensation C2: secret restored after metadata failure',
        failingSecretStore.get('model-provider:del2:api-key') === 'del-secret',
        `got ${JSON.stringify(failingSecretStore.get('model-provider:del2:api-key'))}`,
      );
      const row = db3.prepare("SELECT value FROM settings WHERE key='app'").get() as
        | { value: string }
        | undefined;
      check(
        'Compensation C2: metadata preserved',
        row ? JSON.parse(row.value).models.length === 1 : false,
      );
    }
  }
    ...
    try {
      globalThis.fetch = (async (input, init) => {
        ...
        if (url.includes('api.resume-test.com') || url.includes('api.new-base.com')) {
          lastBaseUrl = url;
          return new Response(
            JSON.stringify({ choices: [{ message: { role: 'assistant', content: 'resumed' } }] }),
            { status: 200 },
          );
        }
        return originalFetch(input, init);
      ...
      await postJSON(
        `${base}/settings/default`,
        { providerId: prov.id, model: 'resume-chat' },
        200,
      );
      ...
      // 修改 provider baseUrl
      await patchJSON(
        `${base}/settings/models/${prov.id}`,
        { baseUrl: 'https://api.new-base.com/v1' },
        200,
      );
      ...
      check(
        'Resume: run completed after baseUrl change',
        status2 === 'completed',
        `status=${status2}`,
      );
      check(
        'Resume: used current baseUrl',
        lastBaseUrl.includes('api.new-base.com'),
        `got ${lastBaseUrl}`,
      );
    ...
    try {
      globalThis.fetch = (async (input, init) => {
        if (String(input).includes('api.slow-echo.com')) {
          ...
          return new Response(
            JSON.stringify({ choices: [{ message: { role: 'assistant', content: 'late' } }] }),
            { status: 200 },
          );
        }
        return originalFetch(input, init);
      ...
      check(
        'Shutdown: run stopped/cancelled',
        finalStatus === 'stopped' || finalStatus === 'interrupted',
        `got ${finalStatus}`,
      );
      cleanupCheckpoint(run.runId);
    } finally {
