...
test('Workspace 目录不存在时恢复被拒', () => {
  ...
  const sessionId = createStoppedSession(manager, 'restore-missing', undefined, {
...
test('Workspace 路径变成普通文件时恢复被拒', () => {
  ...
  const sessionId = createStoppedSession(manager, 'restore-file', undefined, {
  ...
  assert.throws(
    ...
    /Workspace path is no longer a directory/,
  );
  manager.close();
});
...
test('Workspace 路径被 symlink 替换时恢复被拒', () => {
  ...
  const sessionId = createStoppedSession(manager, 'restore-symlink', undefined, {
    workspaceName: 'link-root',
  }).sessionId;
  const session = store.getSession(sessionId, { includeDeleted: true })!;
...
test('realpath 与保存 root 不一致时恢复被拒', () => {
  ...
  fs.symlinkSync(workspaceA, linkRoot);
  const sessionId = createStoppedSession(manager, 'restore-real', undefined, {
    workspaceName: 'real-changed',
  }).sessionId;
  const session = store.getSession(sessionId, { includeDeleted: true })!;
  store.updateSession({ ...session, workspaceRoot: linkRoot });
  manager.deleteWorkspace(sessionId);
...
test('workspaceName 查找不会命中已删除 Workspace', () => {
  ...
  assert.equal(store.findSessionByWorkspaceName('workspace-A'), null);
  assert.equal(
    store.findSessionByWorkspaceName('workspace-A', { includeDeleted: true })?.sessionId,
    's-name',
  );
  store.close();
});
...
test('恢复路径校验不泄露 Host 绝对路径到响应', async () => {
  ...
  const sessionId = createStoppedSession(manager, 'no-path-leak', undefined, {
    workspaceName: 'private-leak',
  }).sessionId;
  const session = store.getSession(sessionId, { includeDeleted: true })!;
