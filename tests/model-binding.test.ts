...
try {
  ...
  check("request model is the run model", bodies[0]?.model === "MiniMax-M3", `got ${bodies[0]?.model}`);
  check("max_tokens from run model capability (16384)", bodies[0]?.max_tokens === 16_384, `got ${bodies[0]?.max_tokens}`);

  // Case 1 + 4：context_usage trace 全部对应当前 Run 模型
  const usage = traces.find((event): event is Extract<TraceEvent, { type: "context_usage" }> => event.type === "context_usage");
  ...
  check("usage.configSource is run_model", usage?.configSource === "run_model", `got ${usage?.configSource}`);
  check("usage.contextWindowTokens is run model capability (512000)",
    usage?.contextWindowTokens === 512_000 && usage?.contextWindowTokens === expected.contextWindowTokens,
    `got ${usage?.contextWindowTokens}`);
  check("usage.maxOutputTokens is run model capability (16384)",
    usage?.maxOutputTokens === 16_384 && usage?.maxOutputTokens === expected.maxOutputTokens,
    `got ${usage?.maxOutputTokens}`);
  check("usage.inputBudgetTokens matches run model budget",
    usage?.inputBudgetTokens === expected.maxInputTokens,
    `got ${usage?.inputBudgetTokens}, want ${expected.maxInputTokens}`);

  // 环境模型能力（32K/4K）未被使用
  check("env model capability (32K/4K) not used anywhere",
