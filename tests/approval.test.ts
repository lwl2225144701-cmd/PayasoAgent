...
const sink: SseSink = {
  write: (chunk: string) => {
    ...
    if (m) {
      try {
        received.push(JSON.parse(m[2]) as HostEvent);
      } catch {
        /* ignore */
      }
    }
...
const reqEv = received.find((e): e is ApprovalRequestedEvent => e.type === 'approval_requested');
check('request() → SSE approval_requested 事件', !!reqEv);
check(
  'approval_requested 含 runId/toolName/args/requestId',
  !!reqEv &&
    reqEv.runId === runId &&
    reqEv.toolName === 'shell' &&
    JSON.stringify(reqEv.args).includes('example.com') &&
    reqEv.requestId.length > 0,
...
// 重复裁决 → false（已删除）
check(
  '重复 resolveApproval → false',
  manager.resolveApproval(runId, reqEv?.requestId ?? 'nope', true) === false,
);
// 未知 requestId → false
...
try {
  ...
  await new Promise((r) => setTimeout(r, 100));
  const req2 = received
    .filter((e): e is ApprovalRequestedEvent => e.type === 'approval_requested')
    .pop();
