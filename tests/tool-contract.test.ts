...
test('execute 注入正确 runId（Tool 从 context 获取）', async () => {
  const res = await execute(
    'probeRunId',
    {},
    { runId: 'RUN-ABC-123', workspaceRoot: TEST_WORKSPACE },
  );
  assert.deepEqual(JSON.parse(res), {
    gotRunId: 'RUN-ABC-123',
    gotWorkspace: TEST_WORKSPACE,
    argRunId: null,
    argWorkspace: null,
  });
});
...
async function main(): Promise<void> {
  ...
  console.log(
    '验收：runId 边界成立（LLM 不可见、Runtime 注入、args 不可覆盖）+ effect/getOperationKey 契约成立 ✓',
  );
}
