...
const docExists =
  fs.existsSync(DOC_PATH) || fs.existsSync(path.join(PROJECT_ROOT, 'docs/architecture-current.md'));
...
if (docTools !== null) {
  ...
  check(
    `工具数量一致 (${expected.length})`,
    expected.length === actualTools.length,
    `文档=${expected.length} 实际=${actualTools.length}`,
  );
}
...
if (docEvents !== null) {
  ...
  check(
    `事件数量一致 (${expected.length})`,
    expected.length === actualEvents.length,
    `文档=${expected.length} 实际=${actualEvents.length}`,
  );
}
