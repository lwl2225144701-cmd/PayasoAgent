...
try {
  // ---- Case 1 + 11 + 12: completed 原子成功 + 重建读取一致 + 非终态事件不受影响 ----
  {
    ...
    check("Case1: exactly one run_completed event",
      events.filter((e) => e.type === "run_completed").length === 1);
    check("Case12: non-terminal events preserved (run_started/llm_call/context_usage)",
      events.some((e) => e.type === "run_started") && events.some((e) => e.type === "llm_call")
        && events.some((e) => e.type === "context_usage"));
    check("Case12: exactly one terminal event among them",
      events.filter((e) => ["run_completed", "run_failed", "run_stopped"].includes(e.type)).length === 1);
    ...
    check("Case11: reload keeps terminal event",
  ...
  // ---- Case 2: failed 原子成功 ----
  {
    ...
    const manager = new RunManager(store);
    seedWorkspace("fz-c2");
    globalThis.fetch = (async () => { throw new Error("injected-failure"); }) as typeof fetch;
    ...
    check("Case2: exactly one run_failed event",
  ...
  // ---- Case 3 + 8: stopping → stopped 原子成功；stop 竞态/重复 stop 幂等 ----
  {
    ...
    check("Case3: exactly one run_stopped event",
      store.listEvents(runId).filter((e) => e.event.type === "run_stopped").length === 1);
    check("Case3: run_stopping 与 run_stopped 分离（stopping 非终态混写）",
      store.listEvents(runId).filter((e) => e.event.type === "run_stopping").length === 1);
    check("Case8: SSE 只广播一次 run_stopped（重复 stop 不重复发布）",
      terminalCount(chunks, "run_stopped") === 1, `chunks=${chunks.filter((c) => c.includes("event: run_")).length}`);
    check("Case8: 无 completed/failed 混入",
  ...
  // ---- Case 6 + 7: 已终态后重复/冲突 finish → no-op ----
  {
    ...
    check("Case7: 不产生 run_stopped", store.listEvents(runId).filter((e) => e.event.type === "run_stopped").length === 0);
    check("Case6: 终态事件仍恰好一次", store.listEvents(runId).filter((e) => e.event.type === "run_completed").length === 1);
  ...
  // ---- Case 9: SSE 严格发生在 durable commit 之后 ----
  {
    ...
    check("Case9: at SSE broadcast time the DB commit already happened",
      statusAtBroadcast === "completed", `got ${statusAtBroadcast}`);
    check("Case9: run_completed SSE delivered exactly once", terminalCount(chunks9, "run_completed") === 1);
    manager.close();
    cleanupDb(dbPath);
  }
  ...
  // ---- Case 4 + 5 + 10: 事务失败注入（SQLite TRIGGER）→ rollback / 不广播 ----
  {
    ...
    await waitFor(() => {
      const n = (new DatabaseSync(dbPath).prepare("SELECT COUNT(*) AS n FROM events WHERE type='llm_call'").get() as { n: number }).n;
      return n > 0;
    });
    await new Promise((r) => setTimeout(r, 400)); // finalize 尝试已完成（失败，已记日志）

    check("Case10: terminal SSE NOT broadcast when finalization fails",
      terminalCount(chunks, "run_completed") === 0);
    check("Case10: in-memory status stays non-terminal (no fake completion)",
      manager.get(runId)?.status === "running", `status=${manager.get(runId)?.status}`);
    const afterFail = new DatabaseSync(dbPath).prepare("SELECT status FROM runs WHERE run_id = ?").get(runId) as { status: string };
    check("Case4: rolled back — Run row remains previous state", afterFail.status === "running", afterFail.status);
    const evCount = new DatabaseSync(dbPath).prepare("SELECT COUNT(*) AS n FROM events WHERE type LIKE 'run_%' AND type != 'run_started' AND type != 'run_stopping'").get() as { n: number };
    ...
    check("Case4: after fault removed, finalization succeeds atomically",
    ...
    store5.createSession({
      sessionId, title: "t", workspaceRoot: "/tmp/x", workspaceName: "w",
      createdAt: "2026-08-30T00:00:00.000Z", updatedAt: "2026-08-30T00:00:00.000Z",
    });
    store5.createRun({
      runId: "r-finalize-5", sessionId, turnIndex: 1, task: "t", status: "running",
      workspaceRoot: "/tmp/x", workspaceName: "w",
      createdAt: "2026-08-30T00:00:00.000Z", updatedAt: "2026-08-30T00:00:00.000Z",
    });
    ...
    `);
    inj5.close();
    assert.throws(() => store5.finalizeRun({
      runId: "r-finalize-5", sessionId, turnIndex: 1, task: "t", status: "completed",
      workspaceRoot: "/tmp/x", workspaceName: "w",
    ...
    check("Case5: run update failure → no terminal event inserted",
      (new DatabaseSync(db5).prepare("SELECT COUNT(*) AS n FROM events").get() as { n: number }).n === 0);
    check("Case5: run status unchanged", (new DatabaseSync(db5).prepare("SELECT status FROM runs WHERE run_id='r-finalize-5'").get() as { status: string }).status === "running");
    store5.close();
    cleanupDb(dbPath);
    cleanupDb(db5);
  }
