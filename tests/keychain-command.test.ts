...
function readArgv(): string[] {
  try { return JSON.parse(fs.readFileSync(argvCapture, "utf8")); } catch { return []; }
}
...
// 5. add 失败时：错误消息不包含原始 stderr / secret
{
  ...
  check("stderr not logged raw", !logs.includes("secret leak?") && !logs.includes("sk-leak-123"), `logs=${logs.slice(0, 120)}`);
...
// 7. 默认 CLI 路径不变（生产）
{
  ...
  check("default CLI is system security", (store as unknown as { cliPath: string }).cliPath === "/usr/bin/security");
}
