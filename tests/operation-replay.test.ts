...
try {
  ...
  runAgentRef = async (task, runId, logs, origLog) => {
    ...
    let error = "";
    try {
      answer = await mod.runAgent(task, undefined, { runId });
    } catch (e) {
  ...
  try {
    assert.equal(sideEffectJournal.length, 1, "期望副作用仅执行 1 次（第二次同 key 被 uncertain 阻断）");
    assert.equal(uncertainHits1, 1, "期望 1 次 [Side-Effect Uncertain]（第二次命中 executing/uncertain）");
  ...
  try {
    ...
    assert.equal(sideEffectJournal.length, journalBefore2, "期望场景2 无副作用发生（execute 次数=0）");
