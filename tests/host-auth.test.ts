...
async function runTests() {
  ...
  try {
    ...
    // 8. 日志不包含 headers/token（间接验证：响应体不含 token）
    {
      ...
      check(
        'response does not leak token',
        !res.body.includes(TEST_TOKEN),
        `body=${res.body.slice(0, 100)}`,
      );
    }
  } finally {
    await host.close();
