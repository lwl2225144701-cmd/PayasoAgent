...
import { register, RequiredRuntimeToolUnavailableError } from "../src/tools/tools.js";
...
try {
  // ---- Harness 单元：refreshToolchain 更新下一轮模型视图 ----
  {
    ...
    check("harness: 默认（无快照）不含可用清单", before.includes("一次性确定") && !before.includes("当前可用工具：git"));
    harness.refreshToolchain(REFRESHED);
    const after = harness.createTranscript("t")[0].content;
    check("harness: refreshToolchain 后模型视图含 git 可用",
  ...
  // ---- Agent 集成：shell 缺依赖 → 准备成功 + 能力刷新 → 下一轮视图更新 ----
  {
    ...
    globalThis.fetch = (async (_input, init) => {
      const body = JSON.parse(String(init?.body)) as { messages: Array<{ role: string; content: string }>, model: unknown };
      bodies.push(body);
      if (bodies.length === 1) {
        return new Response(JSON.stringify({
          choices: [{
            message: {
              role: "assistant",
              content: "",
              tool_calls: [{ id: "call-1", type: "function", function: { name: "shell", arguments: '{"command":"git status"}' } }],
            },
          }],
        }), { status: 200 });
      }
    ...
    const executionContext = { ...createAgentExecutionContext({ runId: "toolchain-refresh" }), toolchain: INITIAL };
    ...
    check("refresh: turn-1 model view had git missing",
      bodies[0]?.messages[0]?.content.includes("当前缺失工具：git") === true);
    check("refresh: turn-2 model view has git available (current Run感知)",
    ...
    check("agent: tool result states no auto-retry (Side-Effect Safety)",
      typeof toolResult?.content === "string" && toolResult.content.includes("not retried automatically"));
