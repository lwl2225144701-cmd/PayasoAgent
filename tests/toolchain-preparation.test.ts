...
const runId = manager.createInSession("toolchain preparation", undefined, { startAgent: false }).runId;
const received: HostEvent[] = [];
const sink: SseSink = {
  write: (chunk) => {
    const match = chunk.match(/data: (.+)\n\n/s);
    if (!match) return;
    try { received.push(JSON.parse(match[1]) as HostEvent); } catch { /* ignore malformed test data */ }
  },
  end: () => {},
  closed: () => false,
};
...
try {
  ...
  try {
    ...
    await new Promise((resolve) => setTimeout(resolve, 20));
    const nextRequest = received.filter((event) => event.type === "toolchain_preparation_requested").pop();
    assert.ok(nextRequest);
    const response = await fetch(`http://127.0.0.1:${portNumber}/runs/${runId}/toolchain-preparation`, {
      method: "POST",
      headers: {
    ...
    await new Promise((resolve) => setTimeout(resolve, 20));
    const cancelRequest = received.filter((event) => event.type === "toolchain_preparation_requested").pop();
    assert.ok(cancelRequest);
    const cancelResponse = await fetch(`http://127.0.0.1:${portNumber}/runs/${runId}/toolchain-preparation`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer toolchain-token",
  ...
  const phaseRunId = phaseManager.createInSession("toolchain preparation phases", undefined, { startAgent: false }).runId;
  const phaseEvents: HostEvent[] = [];
  const phaseSink: SseSink = {
    write: (chunk) => {
      const match = chunk.match(/data: (.+)\n\n/s);
      if (!match) return;
      try { phaseEvents.push(JSON.parse(match[1]) as HostEvent); } catch { /* ignore malformed test data */ }
    },
    end: () => {},
    closed: () => false,
  };
  ...
  try {
    ...
    await new Promise((resolve) => setTimeout(resolve, 20));
    const phaseRequest = phaseEvents.find((event) => event.type === "toolchain_preparation_requested");
    assert.ok(phaseRequest);
    assert.equal(phaseManager.resolveToolchainPreparation(phaseRunId, phaseRequest!.requestId, true), true);
    await new Promise((resolve) => setTimeout(resolve, 20));
    assert.ok(phaseEvents.some((event) => event.type === "toolchain_preparation_started"));
    assert.ok(phaseEvents.some((event) => event.type === "toolchain_preparation_progress" && event.phase === "installing"));
    assert.equal(phaseManager.cancelToolchainPreparation(phaseRunId, phaseRequest!.requestId), true);
    assert.equal((await phasePending).status, "aborted");
    assert.ok(phaseEvents.some((event) => event.type === "toolchain_preparation_resolved" && event.status === "aborted"));
  } finally {
...
// ---- v1.6 闭环③：同 packageName 并发请求合并，只触发一次安装 ----
{
  ...
  const mk = (runId: string) => ({
    runId, toolName: "git" as const, packageName: "git",
    source: "homebrew" as const, timestamp: new Date().toISOString(),
  });
  ...
  const [resultA, resultB] = await Promise.all([
    (async () => {
      ...
      mergeManager.subscribe(runA, { write: (c) => { const m = c.match(/data: (.+)\n\n/s); if (m) eventsA.push(JSON.parse(m[1]) as HostEvent); }, end: () => {}, closed: () => false }, 0, true);
      ...
      assert.equal(mergeManager.resolveToolchainPreparation(runA, requestedA.requestId, true), true);
      return pendingA;
    })(),
  ...
  assert.ok((() => {
    ...
    mergeManager.subscribe(runA, { write: (c) => { const m = c.match(/data: (.+)\n\n/s); if (m) eventsA.push(JSON.parse(m[1])); }, end: () => {}, closed: () => false }, 0, false);
    mergeManager.subscribe(runB, { write: (c) => { const m = c.match(/data: (.+)\n\n/s); if (m) eventsB.push(JSON.parse(m[1])); }, end: () => {}, closed: () => false }, 0, false);
    return eventsA.some((e) => e.type === "toolchain_preparation_resolved")
...
// ---- v1.6 闭环③：拒绝共享 —— 首个被拒绝时，合并等待者同样拿到 denied ----
{
  ...
  await new Promise((resolve) => setTimeout(resolve, 20));
  const eventsA: HostEvent[] = [];
  denyManager.subscribe(runA, {
    write: (c) => { const m = c.match(/data: (.+)\n\n/s); if (m) eventsA.push(JSON.parse(m[1]) as HostEvent); },
    end: () => {}, closed: () => false,
  }, 0, true);
  const requestedA = eventsA.find((e) => e.type === "toolchain_preparation_requested");
  assert.ok(requestedA);
  ...
  assert.ok(resultA.status === "denied" && resultB.status === "denied", "deny merge: 首个拒绝 → 合并等待者共享 denied");
...
// ---- v1.6 闭环④a：快照显示工具已可用 → 短路 prepared，不触发安装 ----
{
  ...
  const availableManager = new RunManager(
    ...
    () => ({
      platform: "macos",
      discovery: "startup",
      tools: { git: { status: "available" }, node: { status: "available" }, npm: { status: "available" } },
    }),
  );
  ...
  const result = await availableManager.toolchainPreparationPort().request({
    runId, toolName: "git", packageName: "git", source: "homebrew", timestamp: new Date().toISOString(),
  });
