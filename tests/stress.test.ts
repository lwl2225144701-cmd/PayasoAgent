...
scenarios['resume-phase2'] = {
  ...
  run: async (ctx) => {
    ...
    return {
      pass,
      detail:
        `resume 后 completedSteps=${completed.length}（恢复前=${resumePreCompleted.length}）, 恢复后完成=${done}, ` +
        `重复步骤号=${dupSteps.length ? dupSteps.join(',') : '无'}, 头部改写/丢步骤=${headChanged}, ` +
        `答案=${answer.slice(0, 80)}`,
      metrics: {
        preSteps: resumePreCompleted.length,
        postSteps: completed.length,
        dupSteps,
        headChanged,
        done,
        toolSeq: calls.map((c) => c.tool),
      },
      gap:
...
scenarios['large-read-700k'] = {
  ...
  run: async (ctx) => {
    ...
    fs.writeFileSync(
      path.join(ws, 'input', 'big.txt'),
      line.repeat(Math.ceil((700 * 1024) / line.length)),
    );
    const task =
  ...
  run: async (ctx) => {
    ...
    return {
      ...
      metrics: {
        readErr: errs.length,
        retries,
        recovery: recoveryCount(out),
        toolSeq: toolCalls(out).map((c) => c.tool),
      },
...
scenarios['recover-flaky-retry'] = {
  ...
  run: async (ctx) => {
    const task =
      '请调用 flakyWrite 工具写入 content=hello，若失败就用完全相同的参数再次调用它，然后告诉我最终结果。';
    const { answer } = await runAgentTask(task, ctx.runId);
    const out = cap.join('\n');
...
scenarios['se-crash-after-effect'] = {
  ...
  run: async (ctx) => {
    ...
    return {
      ...
      gap: pass ? undefined : 'runtime: non_idempotent 崩溃后仍被多次执行（应禁止自动 Retry）',
      layer: pass ? undefined : 'runtime',
    };
  },
};
...
scenarios['se-resume-phase1'] = {
  ...
  run: async (ctx) => {
    ...
    const iv = setInterval(() => {
      ...
      if (
        se.includes('appendEntry::entry:a:x') &&
        se.includes('appendEntry::entry:b:y') &&
...
scenarios['se-resume-phase2'] = {
  ...
  run: async (ctx) => {
    ...
    const ok = counts['entry:a:x'] === 1 && counts['entry:b:y'] === 1 && !!ctx.answer;
    return {
      pass: ok,
...
scenarios['sandbox-traversal'] = {
  ...
  run: async (ctx) => {
    ...
    const checks: [string, () => Promise<unknown>][] = [
      ...
      [
        'readFile ..\\..\\..\\etc\\passwd',
        () => execute('readFile', { path: '..\\..\\..\\etc\\passwd' }, ctx2),
      ],
    ];
    let blocked = 0;
    ...
    const syncChecks: [string, () => unknown][] = [
      ...
      [
        'assert 逃出 sandbox 根',
        () => assertInsideWorkspace(runId, path.join(getSandboxRoot(), '..', 'x')),
      ],
    ];
...
scenarios['sandbox-absolute'] = {
  ...
  run: async (ctx) => {
    ...
    const checks: [string, () => Promise<unknown>][] = [
      ...
      [
        'readFile \\\\server\\share',
        () => execute('readFile', { path: '\\\\server\\share' }, ctx2),
      ],
    ];
    let blocked = 0;
    for (const [label, fn] of checks) if (await expectReject(fn, label, errs)) blocked++;
    if (expectThrow(() => resolvePath(runId, '/etc/passwd'), 'resolvePath /etc/passwd', errs))
      blocked++;
    ...
    for (const [label, fn] of leakChecks) {
      try {
        ...
        if (m.includes('文件不存在') || m.includes('目录不存在') || m.includes('被拒绝'))
          contained++;
...
async function runWorkerScenario(id: string, resumeId?: string): Promise<void> {
  ...
  if (!s) {
    process.stdout.write(
      `[STRESS-RESULT]${JSON.stringify({ pass: false, detail: `unknown scenario: ${id}` })}\n`,
    );
    return;
  }
  ...
  try {
    ...
    res = {
      pass: false,
      detail: `worker 异常: ${(e as Error).message}`,
      metrics: { stack: (e as Error).stack },
    };
  }
  if (res.pass) {
    try {
...
async function runScenarioWorker(
  ...
  const r = spawnSyncCapture(
    ['tsx', '--env-file=.env', 'tests/stress.test.ts', '--scenario', id, ...extra],
    env,
    timeoutMs,
  );
...
async function runOrchestrator(): Promise<void> {
    ...
    try {
      if (id === 'longchain-resume') {
        ...
        if (capRes.pass) {
          ...
          res = {
            pass: false,
            detail: 'longchain-cap 意外 PASS，无法验证 resume（无失败 checkpoint）',
            metrics: capRes.metrics,
          };
        } else {
          const { res: resumeRes } = await runScenarioWorker(
            'resume-phase2',
            ['--resume', capRunId],
            {},
            240_000,
        ...
        const child = spawn(
          ...
          {
            cwd: PROJECT_ROOT,
            env: { ...process.env, ...env, STRESS_RUN_ID: runId },
            stdio: ['ignore', 'pipe', 'pipe'],
            detached: true,
          },
        );
        ...
        const noDup = counts['entry:a:x'] === 1 && counts['entry:b:y'] === 1;
        res = resumeRes.pass
          ? {
              pass: noDup,
  ...
  const dupSideEffect = results.some(
    (r) =>
      JSON.stringify(r.metrics ?? {}).includes('"sideEffectExecutions"') ||
      JSON.stringify(r.metrics ?? {}).includes('重复副作用'),
  );
  ...
  console.log('='.repeat(70));
  console.log(
    `重复副作用: ${dupSideEffect ? '是（se-crash-after-effect / se-resume-crash 见明细）' : '否'}`,
  );
