...
try {
  ...
  const resilientAnswer = await runAgent('observer failure', undefined, {
    ...
    observer: {
      log: () => {
        throw new Error('observer log failed');
      },
      state: () => {
        throw new Error('observer state failed');
      },
      scratchpad: () => {
        throw new Error('observer scratchpad failed');
      },
      traceEvent: () => {
        throw new Error('observer event failed');
      },
      trace: () => {
        throw new Error('observer trace failed');
      },
    },
    modelConfig: {
      baseUrl: 'https://provider.example/v1',
