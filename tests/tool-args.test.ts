...
check("parse: empty string → INVALID_ARGUMENT_JSON", (() => {
  const r = parseToolArguments("");
  return !r.ok && r.error.code === "INVALID_ARGUMENT_JSON";
})());
check("parse: whitespace-only → INVALID_ARGUMENT_JSON", (() => {
  const r = parseToolArguments("   ");
  return !r.ok && r.error.code === "INVALID_ARGUMENT_JSON";
})());
check("parse: truncated JSON → INVALID_ARGUMENT_JSON with stable message", (() => {
  const r = parseToolArguments('{"path":');
  return !r.ok && r.error.code === "INVALID_ARGUMENT_JSON"
    && r.error.message === "Tool arguments are not valid JSON. Retry this tool call with arguments as one valid JSON object.";
})());
for (const bad of ["null", "[]", '"hello"', "123"]) {
  check(`parse: valid JSON but not object (${bad}) → INVALID_ARGUMENTS`, (() => {
    const r = parseToolArguments(bad);
    return !r.ok && r.error.code === "INVALID_ARGUMENTS";
  })());
}
check("parse: valid object → ok", (() => {
...
function streamToolCallResponse(id: string, name: string, rawArguments: string): Response {
  ...
  const chunks = [
    ...
    ...parts.map((p) => 'data: {"choices":[{"delta":{"tool_calls":[{"index":0,"function":{"arguments":' + JSON.stringify(p) + '}}]}}]}\n\n'),
    "data: [DONE]\n\n",
  ];
...
try {
  ...
  // ---- Case 1 + 2 + 3: malformed → 可恢复 tool error → 模型修正 → 执行恰好一次 → completed ----
  {
    ...
    check("Case2: tool never executed with malformed arguments", probeState.executeCount === 1,
      `executeCount=${probeState.executeCount}`);
    check("Case3: LLM corrected args and tool executed exactly once with valid args",
      probeState.lastArgs !== null && (probeState.lastArgs as { path?: string }).path === "test.txt",
      JSON.stringify(probeState.lastArgs));
    const invalid = traces.find((t) => t.type === "tool_call_invalid");
    check("Case1: tool_call_invalid trace emitted with INVALID_ARGUMENT_JSON",
    ...
    check("Case10: completed checkpoint contains paired error tool result",
      !!recoveredCp
      && recoveredCp.messages.some((m) => m.role === "tool" && m.tool_call_id === "call-1" && m.content.includes("INVALID_ARGUMENT_JSON"))
      && recoveredCp.messages.some((m) => m.role === "assistant" && m.tool_calls?.some((c) => c.id === "call-1")));
  ...
  for (const [idx, raw] of ["null", "[]", '"hello"', "123"].entries()) {
    ...
    const invalid = traces.find((t) => t.type === "tool_call_invalid");
    check(`Case4: ${raw} → INVALID_ARGUMENTS, tool not executed, run completed`,
  ...
  // ---- Case 5: unknown tool → TOOL_NOT_FOUND，run 不 crash ----
  {
    ...
    const answer = await runAgent("c5", undefined, {
      runId: "tool-args-unknown",
      workspaceRoot,
      modelConfig: MODEL_CONFIG,
      onTrace: (ev) => traces.push(ev as { type: string; code?: string }),
    });
    const invalid = traces.find((t) => t.type === "tool_call_invalid");
    check("Case5: unknown tool → TOOL_NOT_FOUND error to LLM, run completes",
  ...
  // ---- Case 6 + 7: streaming malformed → 同一恢复路径；streaming 修正后正常 ----
  {
    ...
    globalThis.fetch = (async () => {
      ...
      if (calls === 2) return streamToolCallResponse("call-s2", "args-probe", '{"path":"stream.txt"}'); // streaming 修正
      return streamOkResponse("stream-finished");
    }) as typeof fetch;

    const answer = await runAgent("c6", undefined, {
      runId: "tool-args-stream",
      workspaceRoot,
      modelConfig: MODEL_CONFIG,
      signal: controller.signal,
      onTrace: (ev) => traces.push(ev as { type: string; code?: string }),
    ...
    const invalid = traces.find((t) => t.type === "tool_call_invalid");
    check("Case6: streaming malformed args → same recoverable path (tool_call_invalid)",
      invalid?.code === "INVALID_ARGUMENT_JSON");
    check("Case7: streaming correction executes tool exactly once, run completes",
      answer === "stream-finished" && probeState.executeCount === 1
  ...
  // ---- Case 8: malformed arguments 不创建 side-effect operation（不 started / 不 uncertain）----
  {
    ...
    check("Case8: malformed call → no side-effect operation started/uncertain, tool not executed",
      answer === "done" && nonIdem.executeCount === 0
  ...
  // ---- Case 9: malformed error 之后 abort → 不再调用下一轮 LLM + checkpoint 成对 ----
  {
    ...
    check("Case9: abort checkpoint has no dangling tool result",
