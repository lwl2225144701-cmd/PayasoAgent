...
const shellCall = (n: number, command: string): HostEvent => ({ ...step(n), type: "tool_call", tool: "shell", args: { command } });
const shellError = (n: number): HostEvent => ({ ...step(n), type: "tool_error", tool: "shell", error: "missing tool", attempt: 1, exhausted: true });
const readCall = (n: number): HostEvent => ({ ...step(n), type: "tool_call", tool: "read", args: { path: "a.txt" } });
const readError = (n: number): HostEvent => ({ ...step(n), type: "tool_error", tool: "read", error: "boom", attempt: 1, exhausted: true });

check("无事件 → null", findLastFailedShellCommand([]) === null);
check("只有成功调用、无失败 → null", (() => {
  const r = findLastFailedShellCommand([shellCall(1, "git status")]);
  return r === null;
})());
check("shell 失败 → 提取其命令", (() => {
  const r = findLastFailedShellCommand([shellCall(1, "git status"), shellError(2)]);
  return r === "git status";
})());
check("非 shell 工具的失败不参与提取", (() => {
  const r = findLastFailedShellCommand([readCall(1), readError(2)]);
  return r === null;
})());
check("读失败夹在中间 → 仍取最后失败的 shell 命令", (() => {
...
check("失败后又成功的 shell → 该成功不影响'最后失败'记录", (() => {
  const r = findLastFailedShellCommand([
    shellCall(1, "git status"),
    shellError(2),
    shellCall(3, "ls"),
