        ...
        tool_calls: [
          {
            id: 'c1',
            type: 'function',
            function: { name: 'listDir', arguments: JSON.stringify({ path: 'input' }) },
          },
          {
            id: 'c2',
            type: 'function',
            function: { name: 'readFile', arguments: JSON.stringify({ path: 'input/big.txt' }) },
          },
        ],
      };
    } else {
...
const systemPromptBytes = systemMsg
  ? Buffer.byteLength(String(systemMsg.content ?? ''), 'utf8')
  : 0;
...
console.log(
  `guardedResultBytes（Output Guard 后，trace returnedBytes）: ${truncEvents.length ? kb(truncEvents[0].returnedBytes) : '无截断事件'}`,
);
...
console.log(
  `Agent 是否崩溃                         : ${agentErr ? `是（${agentErr.slice(0, 60)}）` : '否'}`,
);
...
try {
  ...
  assert.ok(
    truncEvents[0].returnedBytes <= MAX_TOOL_OUTPUT_BYTES,
    'guarded returnedBytes 应 <=16KB',
  );
  assert.ok(
    scratchpadBytes <= MAX_TOOL_OUTPUT_BYTES + 64,
    'scratchpad 不应再保存 700KB（应 <=16KB）',
  );
  assert.ok(
    systemPromptBytes <= MAX_TOOL_OUTPUT_BYTES * 2 + 64,
    'system prompt 应 <= ~32KB（16KB×2 容忍）',
  );
