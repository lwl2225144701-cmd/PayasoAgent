...
async function waitTerminal(id: string, ms = 120000): Promise<any> {
  ...
  while (Date.now() - t0 < ms) {
    ...
    if (body.status === 'completed' || body.status === 'failed' || body.status === 'stopped')
      return body;
    await wait(500);
  }
...
check(
  'POST /runs → 202 + runId',
  r1.status === 202 && typeof r1.body.runId === 'string' && r1.body.runId.length > 0,
  `status=${r1.status}`,
);
...
check(
  'GET /sessions/:id/runs 返回首轮',
  sessionRuns.runs?.[0]?.runId === id1 && sessionRuns.runs?.[0]?.turnIndex === 1,
);
...
check(
  '文件列表包含对应 run 的 marker(B)',
  filesB.files.some((f: any) => f.name === 'work/marker.txt'),
);
check(
  '文件列表不串状态(A 无 marker)',
  !filesA.files.some((f: any) => f.name === 'work/marker.txt'),
);
...
check(
  'GET /runs/:id → 200 + status(running/completed)',
  g1.status === 200 && ['running', 'completed'].includes(g1.body.status),
  `status=${g1.status}`,
);
const list = await (await fetch(`${base}/runs`)).json();
check(
  'GET /runs 包含该 run',
  Array.isArray(list.runs) && list.runs.some((r: any) => r.runId === id1),
);
...
check(
  'SSE 收到 assistant_delta',
  events.some((e) => e.type === 'assistant_delta'),
);
check(
  'SSE 收到 tool_call',
...
check(
  'Run 最终 completed + 有 result',
  final1.status === 'completed' && typeof final1.result === 'string' && final1.result.length > 0,
