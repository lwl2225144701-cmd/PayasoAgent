...
test('moveFile：./work/m/n.txt 与 work/m/n.txt 源/目标归一化为同一 identity', () => {
  ...
  const a = operationIdentity(t, { source: './work/m/n.txt', target: './work/t/n.txt' }, ctx);
...
test('moveFile 防重放：成功执行后同 identity 请求回放（idle），源不重复移动', async () => {
  ...
  // 首次：无回放 → 真实执行 + 记录
  assert.equal(
    getReplay(g, t, { source: 'work/idle-src.txt', target: 'work/idle-dst.txt' }, ctx),
    undefined,
  );
  const r1 = await execute(
    'moveFile',
    { source: 'work/idle-src.txt', target: 'work/idle-dst.txt' },
    ctx,
  );
